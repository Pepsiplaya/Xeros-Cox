...
public class GUI {
	...
	public GUI(AIOSmelter smelt) {
		try {
			...
			frame.setTitle("AIO Smelter");
			frame.setBounds(100, 100, 220, 210);
			contentPane = new JPanel();
			contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
			frame.setContentPane(contentPane);
			contentPane.setLayout(null);
			frame.setLocationRelativeTo(smelt.ctx.mouse.getComponent());
			initComponents();
			frame.repaint();
	...
	public void onApplyNewSettings() {
		...
		smelt.setupOres(oreType.getItemId());
		smelt.setupOres2(oreType.getItemId2());
		smelt.setupQuantity(oreType.getQuantity());
		smelt.setupQuantity2(oreType.getQuantity2());
		smelt.setupData(oreType.getData());
		smelt.started = true;
	}
